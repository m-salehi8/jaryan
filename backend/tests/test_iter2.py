...
class TestSeedIteration2:
    def test_forms_include_services_tabs_form(self, session, designer_token):
        ...
        assert target, f"missing form 'فرم درخواست خدمات (پشتیبانی)'. Names: {[f['name'] for f in forms]}"
        full = session.get(f"{BASE_URL}/api/forms/{target['id']}", headers=auth_headers(designer_token)).json()
        fields = full["fields"]
        ...
        assert len(tabs_field.get("tab_options", [])) >= 5, f"tab_options should have 5+, got {len(tabs_field.get('tab_options', []))}"
        for t in tabs_field["tab_options"]:
            assert "id" in t and "label" in t
        # Has children referencing parent_tab_field_id + parent_tab_id
        children = [f for f in fields if f.get("parent_tab_field_id") == tabs_field["id"] and f.get("parent_tab_id")]
        assert len(children) >= 2, f"expected child fields under tabs, found {len(children)}"

    def test_petty_workflow_has_conditional_edge(self, session, admin_token):
        rows = session.get(f"{BASE_URL}/api/workflows", headers=auth_headers(admin_token)).json()
        petty = next((w for w in rows if w["name"] == "فرایند درخواست تنخواه"), None)
        assert petty, "missing تنخواه workflow"
        full = session.get(f"{BASE_URL}/api/workflows/{petty['id']}", headers=auth_headers(admin_token)).json()
...
class TestFormTabsAndVisibility:
    def test_create_form_with_tabs_and_visible_if_roundtrip(self, session, designer_token):
        payload = {
            "name": "TEST_فرم تب‌دار",
            "description": "تست تب و شرط",
            "fields": [
                {"id": "ftabs", "type": "tabs", "label": "نوع",
                 "tab_options": [{"id": "t1", "label": "نوع ۱"}, {"id": "t2", "label": "نوع ۲"}]},
                {"id": "fchild1", "type": "text", "label": "متن ۱",
                 "parent_tab_field_id": "ftabs", "parent_tab_id": "t1"},
                {"id": "fchild2", "type": "text", "label": "متن ۲",
                 "parent_tab_field_id": "ftabs", "parent_tab_id": "t2"},
                {"id": "fselect", "type": "select", "label": "نمایش؟", "options": ["yes", "no"]},
                {"id": "fcond", "type": "text", "label": "شرطی",
                 "visible_if": {"field_id": "fselect", "op": "=", "value": "yes"}},
            ],
        }
        r = session.post(f"{BASE_URL}/api/forms", json=payload, headers=auth_headers(designer_token))
        assert r.status_code == 200, r.text
        fid = r.json()["id"]
        # PATCH: modify visible_if and verify persistence
        new_fields = payload["fields"]
        new_fields[-1]["visible_if"] = {"field_id": "fselect", "op": "!=", "value": "no"}
        r2 = session.patch(f"{BASE_URL}/api/forms/{fid}", json={"fields": new_fields},
                           headers=auth_headers(designer_token))
        assert r2.status_code == 200, r2.text
        # GET and verify all special fields preserved
        full = session.get(f"{BASE_URL}/api/forms/{fid}", headers=auth_headers(designer_token)).json()
        ...
        # cleanup
        session.delete(f"{BASE_URL}/api/forms/{fid}", headers=auth_headers(designer_token))
...
class TestEvaluateRule:
    ...
    def test_eq_numeric_coercion(self):
        ...
        assert evaluate_rule({"field_id": "x", "op": "=", "value": "5"}, {"x": 5}) is True
        assert evaluate_rule({"field_id": "x", "op": "=", "value": "5"}, {"x": "6"}) is False

    def test_gt_lt_numeric(self):
        assert evaluate_rule({"field_id": "amt", "op": ">", "value": "5000000"}, {"amt": "8000000"}) is True
        assert evaluate_rule({"field_id": "amt", "op": ">", "value": "5000000"}, {"amt": "1000000"}) is False
        assert evaluate_rule({"field_id": "amt", "op": "<=", "value": "5000000"}, {"amt": "5000000"}) is True

    def test_contains_string(self):
        assert evaluate_rule({"field_id": "s", "op": "contains", "value": "abc"}, {"s": "xxabcyy"}) is True
        assert evaluate_rule({"field_id": "s", "op": "contains", "value": "zz"}, {"s": "xxabcyy"}) is False
...
def _get_tasks_for(session, token, process_id=None):
    rows = session.get(f"{BASE_URL}/api/tasks?assigned_to_me=true", headers=auth_headers(token)).json()
    if process_id:
...
def _start_petty(session, designer_token):
    ...
    petty = session.get(f"{BASE_URL}/api/workflows/{petty_summary['id']}",
                        headers=auth_headers(designer_token)).json()
    r = session.post(f"{BASE_URL}/api/workflows/{petty['id']}/start", headers=auth_headers(designer_token))
    assert r.status_code == 200, r.text
    body = r.json()
    ...
    assert advanced.get("next_tasks"), f"start_workflow did not create first task: {body}"
    return petty, inst
...
def _amount_field_id(petty_workflow, session, designer_token):
    ...
    form_node = next((n for n in petty_workflow["nodes"] if n.get("type") == "form"), None)
    ...
    form = session.get(f"{BASE_URL}/api/forms/{form_id}", headers=auth_headers(designer_token)).json()
    # Find a numeric field whose id matches the conditional edge field_id
    return form, form_node


class TestEngineE2E:
    def test_above_threshold_traverses_finance_approval(self, session, designer_token, employee_token,
        ...
        r = session.patch(f"{BASE_URL}/api/tasks/{emp_task['id']}",
        ...
        r2 = session.patch(f"{BASE_URL}/api/tasks/{mgr_task['id']}",
        ...
        assert admin_task, f"admin has no task after manager approval (next ids were {next_ids})"
        r3 = session.patch(f"{BASE_URL}/api/tasks/{admin_task['id']}",
        ...
        r4 = session.patch(f"{BASE_URL}/api/tasks/{pay_task['id']}",
                           json={"status": "done"}, headers=auth_headers(pay_token))
        assert r4.status_code == 200, r4.text
        b4 = r4.json()
        # Verify process completed
        proc = session.get(f"{BASE_URL}/api/processes/{pid}", headers=auth_headers(admin_token)).json()
        assert proc["process"]["status"] == "completed", f"process not completed: {proc['process']['status']}"

    def test_below_threshold_skips_finance(self, session, designer_token, employee_token,
                                            manager_token, admin_token):
        petty, inst = _start_petty(session, designer_token)
        pid = inst["id"]
        ...
        emp_task = _find_pending_task_for(session, employee_token, pid)
        assert emp_task
        session.patch(f"{BASE_URL}/api/tasks/{emp_task['id']}",
        ...
        assert bypass, f"finance (n4={n4_target}) should be skipped, got: {next_after_mgr}"

    def test_rejection_stops_process(self, session, designer_token, employee_token, manager_token):
        petty, inst = _start_petty(session, designer_token)
        pid = inst["id"]
        cond_edge = next(e for e in petty["edges"] if e.get("condition"))
        amount_field_id = cond_edge["condition"]["field_id"]
        ...
        mgr_task = _find_pending_task_for(session, manager_token, pid)
        assert mgr_task
        r = session.patch(f"{BASE_URL}/api/tasks/{mgr_task['id']}",
                          json={"status": "rejected"}, headers=auth_headers(manager_token))
        assert r.status_code == 200, r.text
        proc = session.get(f"{BASE_URL}/api/processes/{pid}", headers=auth_headers(designer_token)).json()
        assert proc["process"]["status"] == "rejected", f"expected rejected, got {proc['process']['status']}"
        ...
        assert "pending" not in statuses or all(
            t["node_id"] in (emp_task["node_id"], mgr_task["node_id"]) for t in tasks if t["status"] == "pending"
