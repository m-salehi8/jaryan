...
class TestEvaluateGroupRule:
    def test_and_group_all_match(self):
        ...
        assert (
            evaluate_rule(rule, {"_task_status": "approved", "amount": "8000000"})
            is True
        )
    ...
    def test_and_group_one_fails(self):
        ...
        assert (
            evaluate_rule(rule, {"_task_status": "approved", "amount": "1000000"})
            is False
        )
    ...
    def test_or_group_one_match(self):
        ...
        # only second matches
        assert (
            evaluate_rule(rule, {"_task_status": "rejected", "amount": "8000000"})
            is True
        )
        # neither matches
        assert (
            evaluate_rule(rule, {"_task_status": "rejected", "amount": "1000000"})
            is False
        )
    ...
    def test_nested_group(self):
        ...
        assert (
            evaluate_rule(rule, {"a": "1", "b": "2", "c": "0"}) is True
        )  # inner AND matches
        assert (
            evaluate_rule(rule, {"a": "1", "b": "9", "c": "3"}) is True
        )  # outer C matches
        assert (
            evaluate_rule(rule, {"a": "1", "b": "9", "c": "0"}) is False
        )  # neither matches
    ...
    def test_single_clause_backwards_compat(self):
        ...
        assert (
            evaluate_rule({"field_id": "x", "op": ">", "value": "10"}, {"x": "5"})
            is False
        )
...
class TestGroupRuleRoundTrip:
    def test_edge_group_rule_persists_through_patch(self, session, designer_token):
        ...
        assert (
            c0["field_id"] == "_task_status"
            and c0["op"] == "="
            and c0["value"] == "approved"
        )
...
def _publish_and_start(session, designer_token, payload):
    r = session.post(
        f"{BASE_URL}/api/workflows", json=payload, headers=auth_headers(designer_token)
    )
    assert r.status_code == 200, r.text
    wf_id = r.json()["id"]
    r2 = session.patch(
    ...
    assert r2.status_code == 200
    r3 = session.post(
        f"{BASE_URL}/api/workflows/{wf_id}/start", headers=auth_headers(designer_token)
    )
    assert r3.status_code == 200, r3.text
...
class TestE2EGroupEdges:
    def test_and_group_taken_when_both_match(
        self, session, designer_token, employee_token, manager_token
    ):
        wf_id, proc = _publish_and_start(
            session, designer_token, _build_petty_with_group("and")
        )
        pid = proc["id"]
        ...
        assert (
            "n4" in next_ids
        ), f"expected n4 (admin) in next_tasks (AND matched), got: {next_ids}"
        session.delete(
            f"{BASE_URL}/api/workflows/{wf_id}", headers=auth_headers(designer_token)
        )

    def test_and_group_skipped_when_one_fails(
        self, session, designer_token, employee_token, manager_token
    ):
        wf_id, proc = _publish_and_start(
            session, designer_token, _build_petty_with_group("and")
        ...
        assert (
            "n4" not in next_ids
        ), f"n4 should be skipped (AND group failed): {next_ids}"
        assert "n5" in next_ids, f"expected default n5 (payment), got: {next_ids}"
        session.delete(
            f"{BASE_URL}/api/workflows/{wf_id}", headers=auth_headers(designer_token)
        )

    def test_or_group_taken_when_one_matches(
        self, session, designer_token, employee_token, manager_token
    ):
        wf_id, proc = _publish_and_start(
            session, designer_token, _build_petty_with_group("or")
        )
        pid = proc["id"]
        emp = _find_pending(session, employee_token, pid)
        assert emp
        ...
        assert (
            "n4" in next_ids
        ), f"expected n4 in next_tasks (OR matched via _task_status), got: {next_ids}"
        session.delete(
            f"{BASE_URL}/api/workflows/{wf_id}", headers=auth_headers(designer_token)
        )
