...
class TestDashboard:
    def test_dashboard_shape(self, session, admin_token):
        ...
        assert r.status_code == 200
        data = r.json()
        for k in ("counters", "my_tasks", "pending_approvals", "running_processes", "activities", "recommendations"):
    ...
    def test_dashboard_manager_has_approvals(self, session, manager_token):
        r = session.get(f"{BASE_URL}/api/dashboard", headers=auth_headers(manager_token))
        assert r.status_code == 200
        d = r.json()
...
class TestWorkflows:
    ...
    def test_detail(self, session, admin_token):
        ...
        r = session.get(f"{BASE_URL}/api/workflows/{wf['id']}", headers=auth_headers(admin_token))
        assert r.status_code == 200
        d = r.json()
        assert d["id"] == wf["id"]
    ...
    def test_create_and_update_and_start(self, session, designer_token):
        payload = {
            ...
            "nodes": [
                ...
                {"id": "n2", "type": "end", "label": "پایان", "position": {"x": 340, "y": 120}, "data": {}},
            ],
        ...
        # PATCH publish
        upd = {"status": "published", "nodes": payload["nodes"], "edges": payload["edges"]}
        r2 = session.patch(f"{BASE_URL}/api/workflows/{wf_id}", json=upd, headers=auth_headers(designer_token))
        assert r2.status_code == 200, r2.text
        assert r2.json()["status"] == "published"

        # Verify GET persisted
        r3 = session.get(f"{BASE_URL}/api/workflows/{wf_id}", headers=auth_headers(designer_token))
        assert r3.status_code == 200
        assert r3.json()["status"] == "published"

        # Start instance
        r4 = session.post(f"{BASE_URL}/api/workflows/{wf_id}/start", headers=auth_headers(designer_token))
...
class TestForms:
    ...
    def test_crud(self, session, designer_token):
        ...
        fid = f["id"]
        # GET
        r2 = session.get(f"{BASE_URL}/api/forms/{fid}", headers=auth_headers(designer_token))
        assert r2.status_code == 200
        # PATCH
        r3 = session.patch(f"{BASE_URL}/api/forms/{fid}", json={"name": "TEST_فرم ویرایش‌شده"},
...
class TestTasks:
    def test_manager_assigned_tasks(self, session, manager_token):
        r = session.get(f"{BASE_URL}/api/tasks?assigned_to_me=true", headers=auth_headers(manager_token))
        assert r.status_code == 200
        tasks = r.json()
    ...
    def test_task_approve_creates_activity(self, session, manager_token):
        rows = session.get(f"{BASE_URL}/api/tasks?assigned_to_me=true&status=pending",
                           headers=auth_headers(manager_token)).json()
        if not rows:
            # All seeded pendings already consumed by previous runs; fall back to any of manager's tasks
            all_rows = session.get(f"{BASE_URL}/api/tasks?assigned_to_me=true",
                                   headers=auth_headers(manager_token)).json()
            if not all_rows:
            ...
            task = rows[0]
        r = session.patch(f"{BASE_URL}/api/tasks/{task['id']}", json={"status": "approved"},
        ...
        dash = session.get(f"{BASE_URL}/api/dashboard", headers=auth_headers(manager_token)).json()
        acts = dash["activities"]
        assert any(a["target_id"] == task["id"] and "approved" in a["action"] for a in acts)
...
class TestComments:
    def test_create_and_list(self, session, designer_token):
        ...
        r = session.post(f"{BASE_URL}/api/comments",
        ...
        r2 = session.get(f"{BASE_URL}/api/comments?target_type=node&target_id={target_id}",
                         headers=auth_headers(designer_token))
        assert r2.status_code == 200
        rows = r2.json()
        assert len(rows) == 1
...
class TestProcesses:
    def test_list_and_detail(self, session, admin_token):
        rows = session.get(f"{BASE_URL}/api/processes", headers=auth_headers(admin_token)).json()
        assert len(rows) >= 1
        running = [p for p in rows if p["status"] == "running"]
        assert len(running) >= 1
        # Pick a process whose workflow still exists (seeded one)
        wfs = session.get(f"{BASE_URL}/api/workflows", headers=auth_headers(admin_token)).json()
        ...
        pid = candidates[0]["id"]
        r = session.get(f"{BASE_URL}/api/processes/{pid}", headers=auth_headers(admin_token))
        assert r.status_code == 200
        d = r.json()
...
class TestAIStream:
    def test_ai_generate_workflow_sse(self, session, designer_token):
        ...
        headers = {**auth_headers(designer_token), "Content-Type": "application/json", "Accept": "text/event-stream"}
