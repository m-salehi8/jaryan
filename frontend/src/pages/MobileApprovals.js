  ...
  return (
    <div className="min-h-screen bg-muted max-w-md mx-auto" data-testid="mobile-root">
      <header className="bg-card border-b border-border px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <button onClick={() => { logout(); window.location.href = "/login"; }} className="p-2 text-muted-foreground">
      ...
      <div className="p-4">
        <div className="mb-4 text-xs text-muted-foreground">
          سلام <span className="font-medium text-foreground">{user?.full_name}</span> — {tasks.length === 0 ? "همه چیز مرتب است." : `${tasks.length} تسک منتظر شماست.`}
        </div>

        {loading && <div className="text-center text-sm text-muted-foreground py-12">در حال بارگذاری…</div>}

        {!loading && tasks.length === 0 && (
          <div className="border border-dashed border-border rounded-xl p-10 text-center" data-testid="mobile-empty">
            <Inbox className="w-8 h-8 text-neutral-300 mx-auto mb-3" />
            <div className="text-sm text-muted-foreground">هیچ تسکی در انتظار نیست.</div>
          </div>
        )}

        <ul className="space-y-2">
          {tasks.map(t => (
            <li key={t.id} className="bg-card rounded-xl border border-border p-4 active:scale-[0.99] transition" data-testid={`m-task-${t.id}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] text-muted-foreground mb-1">{t.workflow_name}</div>
                  <div className="text-sm font-semibold text-foreground">{t.title}</div>
                  {t.description && <div className="text-xs text-muted-foreground mt-1.5 leading-6 line-clamp-2">{t.description}</div>}
                ...
                <button onClick={() => setActive(t)} className="p-2 text-muted-foreground" data-testid={`m-detail-${t.id}`}>
              ...
              <div className="flex gap-2 mt-3">
                {t.type === "approval" ? (
                  ...
                  <button data-testid={`m-done-${t.id}`} onClick={() => act(t.id, "done")} className="flex-1 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium flex items-center justify-center gap-1.5">
      ...
      {active && (
        <div className="fixed inset-0 bg-black/40 z-20 flex items-end" onClick={() => setActive(null)}>
          <div className="bg-card w-full rounded-t-2xl p-5 animate-in" onClick={(e) => e.stopPropagation()}>
            <div className="w-10 h-1 bg-neutral-200 rounded-full mx-auto mb-4" />
            <div className="text-[10px] text-muted-foreground">{active.workflow_name}</div>
            <h3 className="text-base font-semibold mt-1">{active.title}</h3>
            {active.description && <p className="text-sm text-muted-foreground mt-2 leading-7">{active.description}</p>}
