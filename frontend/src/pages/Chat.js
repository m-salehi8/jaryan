        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {messages.length === 0 && (
            <div className="text-center py-12 animate-in" data-testid="chat-empty">
              ...
              <h2 className="text-2xl font-semibold text-neutral-900">یک فرایند جدید بسازیم؟</h2>
              ...
              <div className="mt-8 grid sm:grid-cols-2 gap-2 max-w-xl mx-auto">
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    onClick={() => send(s)}
                    className="text-right px-4 py-3 rounded-lg border border-neutral-200 hover:border-brand hover:bg-brand-soft hover:text-brand transition-colors text-sm text-neutral-700"
                  >
                    {s}
                  </button>
                ))}
              ...
              {m.role === "user" ? (
                ...
                <div className="me-auto max-w-[80%]">
                  ...
                  {m.generated_workflow && (
                    <div className="mt-3 bg-white border border-neutral-200 rounded-xl p-4" data-testid="workflow-preview">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <div className="text-xs text-neutral-400">پیش‌نمایش فرایند</div>
                      ...
                      <div className="flex items-center gap-2 overflow-x-auto pb-2">
                        {m.generated_workflow.nodes?.map((n, idx) => (
                          <div key={n.id} className="flex items-center gap-2 shrink-0">
                            <div className="border border-neutral-200 rounded-lg px-3 py-2 min-w-[140px]">
                              <div className="text-[10px] text-neutral-400 mono uppercase">{n.type}</div>
                              <div className="text-xs font-medium text-neutral-900">{n.label}</div>
                            </div>
          ...
          {streaming && (
            <div className="flex items-center gap-2 text-neutral-400 text-xs ps-9">
      ...
      <div className="border-t border-neutral-200 bg-white px-4 md:px-6 py-4">
        <div className="max-w-3xl mx-auto">
          <form
            ...
            className="flex items-center gap-2 rounded-xl border border-neutral-200 focus-within:border-neutral-900 bg-white px-3"
          >
            <input
